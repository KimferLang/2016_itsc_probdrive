...
type DynamicBayesianNetworkBehavior <: AbstractVehicleBehavior
    ...
    function DynamicBayesianNetworkBehavior(
        model::DBNModel,
        simparams_lat::DBNSimParams, #TODO(tim): get rid of these?
        simparams_lon::DBNSimParams,
...
ProbDrive2016ITSC.get_targets(behavior::DynamicBayesianNetworkBehavior) = ModelTargets(symbol2feature(behavior.symbol_lat), Features.symbol2feature(behavior.symbol_lon))
ProbDrive2016ITSC.get_indicators(behavior::DynamicBayesianNetworkBehavior) = behavior.extractor.indicators
